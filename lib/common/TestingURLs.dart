//广西卫视：
const RTMP_GX = "rtmp://58.200.131.2:1935/livetv/gxtv";
// 湖南卫视：
const RTMP_HN = "rtmp://58.200.131.2:1935/livetv/hunantv";

// 一段动画
const RTSP = "rtsp://wowzaec2demo.streamlock.net/vod/mp4:BigBuckBunny_115k.mov";
//一段录播
const FLV_GAME = "http://1011.hlsplay.aodianyun.com/demo/game.flv";
//西瓜播放器测试视频
const FLV_XG_360 =
    "https://sf1-hscdn-tos.pstatp.com/obj/media-fe/xgplayer_doc_video/flv/xgplayer-demo-360p.flv";
const FLV_720 =
    "https://sf1-cdn-tos.huoshanstatic.com/obj/media-fe/xgplayer_doc_video/flv/xgplayer-demo-720p.flv";
const FLV_480 =
    "https://sf1-cdn-tos.huoshanstatic.com/obj/media-fe/xgplayer_doc_video/flv/xgplayer-demo-480p.flv";
const FLV_360 =
    "https://sf1-cdn-tos.huoshanstatic.com/obj/media-fe/xgplayer_doc_video/flv/xgplayer-demo-360p.flv";
//西瓜mp4
const MP4_360_1 =
    "https://sf1-hscdn-tos.pstatp.com/obj/media-fe/xgplayer_doc_video/mp4/xgplayer-demo-360p.mp4";
const MP4_720 =
    "https://sf1-cdn-tos.huoshanstatic.com/obj/media-fe/xgplayer_doc_video/mp4/xgplayer-demo-720p.mp4";
const MP4_480 =
    "https://sf1-cdn-tos.huoshanstatic.com/obj/media-fe/xgplayer_doc_video/mp4/xgplayer-demo-480p.mp4";
const MP4_360 =
    "https://sf1-cdn-tos.huoshanstatic.com/obj/media-fe/xgplayer_doc_video/mp4/xgplayer-demo-360p.mp4";
//OBS
// final OBS_URL_RTMP = "rtmp://192.168.1.105:1985/myapp/test";
// final OBS_URL_FLV =
//     "http://192.168.1.105:80/live.flv?port=1985&app=myapp&stream=test";
const OBS_URL_RTMP = "rtmp://193.112.200.88:1985/myapp/test";
const OBS_URL_FLV =
    "http://193.112.200.88:80/live.flv?port=1985&app=myapp&stream=test";
const SRS="http://193.112.200.88:8080/live/livestream.flv";
const FFmpeg="http://localhost:80/live.flv?port=1985&app=myapp&stream=test";